...
function validate(data, done) {
  ...
  /**
   * returning an error in async stops async from continuing, so instead push
   * errors to errors array
   */
  ...
  function validateProperty(property, submittedValue, done) {
    ...
    async.forEachOf(validations, function(value, key, next) {
      if (validator.isCustomFunction(value)) {
        ...
        errors.push(new Error(k + ' validation is not defined'));
        return next();
      } else {
        if (validator[key](submittedValue, validations[key])) {
          return next()
        } else {
          errors.push(new Error(property + ': ' + submittedValue + ' did not pass '
                                + key +' validation'));
          return next();
        }
      }
